window.ContextHubKernelConfig = {
    ...
    "stores": {
        ...
        "orderhistory": {
            "type": "commerce.orderhistory",
            "required": true,
            "config": {"service": {"secure": true}}
            }
        }
    };
/*!
...
(function($, window) {
     ...
     * @constructor
     */
    var GeolocationBasicStore = function(name, config) {
    ...
    ContextHub.Utils.inheritance.inherit(GeolocationBasicStore, ContextHub.Store.PersistedStore);
     ...
     * @return {$.Deferred}
     */
    GeolocationBasicStore.prototype.getCoordinates = function() {
        var result = $.Deferred();
     ...
     */
    GeolocationBasicStore.prototype.loadCurrentLocation = function() {
        var self = this;
     ...
     */
    GeolocationBasicStore.prototype.loadLocation = function(coordinates) {
     ...
     */
    GeolocationBasicStore.prototype.storeLocation = function(location, defaults) {
     ...
     * @return {Boolean}
     */
    GeolocationBasicStore.prototype.areCoordinatesSet = function() {
     ...
     * Updates module.
     */
    GeolocationBasicStore.prototype.updateLocationModule = function() {
        var self = this;
    ...
    ContextHub.Utils.storeCandidates.registerStoreCandidate(GeolocationBasicStore, 'contexthub.geolocation', 10, function() {
...
(function($, window) {
    ...
    var GeolocationStore = function(name, config) {
        ...
        /* query service */
        // removed the initial queryService call because we already have the address for the initial coordinates
    ...
    /**
     * Generates signature required for Google Maps API usage.
     *
     * @private
...
(function($) {
    ...
    var defaultConfig = {
        mappingConfig: {
            ...
            clientContextStoreName: 'geolocation',
     ...
     * @param {Object} config - config
     */
    var ClientContextGeolocation = function(name, config) {
        this.config = $.extend(true, {}, defaultConfig, config);
        this.init(name, this.config);
        this.queryService(true);
    };

    ContextHub.Utils.inheritance.inherit(ClientContextGeolocation, ContextHub.Store.ClientContextBackedStore);

    /**
     * Loads given location in ClientContext.
     *
     * @param {Object} location - location
     */
    ClientContextGeolocation.prototype.loadLocation = function(location) {
        var store = ClientContext.get(this.config.mappingConfig.clientContextStoreName);

        if (store) {
            store.setProperties(location);
            store.fireEvent('update');
        }
    };

    /**
     * Resets ClientContext's geolocation store.
     */
    ClientContextGeolocation.prototype.reset = function() {
        var store = ClientContext.get(this.config.mappingConfig.clientContextStoreName);
    ...
    /* register ClientContext - ContextHub geolocation connector */
    ContextHub.Utils.storeCandidates.registerStoreCandidate(ClientContextGeolocation, 'contexthub.geolocation', 20, function() {
...
(function ($) {
    ...
    var defaultConfig = {
        service: {
            ...
            timeout: 1000,
            ttl: 0 * 60 * 1000,
            path: '${resourcePath}.commerce.orderhistory.json'
        }
    };

    var OrderHistoryStore = function(name, config) {
        this.config = $.extend({}, defaultConfig, config);
        this.init(name, this.config);
        this.queryService(false);

        //
        // Update the order history if the user changes.
        //
        var that = this;
        ContextHub.eventing.on(ContextHub.Constants.EVENT_STORE_UPDATED + ":profile", function(event, data) {
                that.queryService(true);
            });
    };

    ContextHub.Utils.inheritance.inherit(OrderHistoryStore, ContextHub.Store.JSONPStore);

    /**
     * Resolve any "${resourcePath}" references in the service path.
     */
    OrderHistoryStore.prototype.getServiceDetails = function() {
        if (this.config.service.path) {
            var resourcePath = ContextHub.Paths.RESOURCE_PATH;
            this.config.service.path = this.config.service.path.replace(/\${resourcePath}/g, resourcePath);
        }

        return this.config.service;
    };

    OrderHistoryStore.prototype.successHandler = function(response) {
        var _ = this.getItem("_");
        this.setItem('/', response);
        this.setItem('_', _);
    };

    ContextHub.Utils.storeCandidates.registerStoreCandidate(OrderHistoryStore, 'commerce.orderhistory', 0);
